...
from validation import assert_supported_params
...
class Layer:
    ...
    def add_bias(self, x: Tensor) -> Tensor:
        if self.has_bias:
            return x.v_append(Tensor([1 for _ in range(len(x))]))
        else:
            return x
    ...
    def init_weights(self, w_init_strat: str, has_bias: bool) -> Tensor:
        ...
        return Tensor(generate_weights(shape_of_weights))


lay1 = Layer(3, 'relu', 5, w_init_strat='ones')
lay2 = Layer(2, 'relu', 3)
result = lay2.forward(lay1.forward(Tensor([1, 2, 3, 4, 5])))
